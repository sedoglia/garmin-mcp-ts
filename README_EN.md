...
[![Version](https://img.shields.io/badge/Version-4.5.10-green.svg)](https://github.com/sedoglia/garmin-mcp-ts/releases/latest)
...
> 📦 **Latest release: [v4.5.10](https://github.com/sedoglia/garmin-mcp-ts/releases/latest)**
> — the `.mcpb` bundle ready to install, with the release notes, the `.sha256` file to
> verify it and the [SBOM](#-sbom-software-bill-of-materials) in CycloneDX format.
>
...
Next to the bundle, every release also publishes `garmin-mcp-ts.sbom.cdx.json`, the full
list of dependencies the bundle contains: see
...
The published bundle is built by the `Release` workflow, which attaches `.mcpb`,
`.sha256` and the CycloneDX SBOM to a draft release. It starts either way:
...
`releases/latest` instead and never need touching. Once the version is raised, regenerate
the SBOM too with `npm run sbom`: the root component carries the number, and `check:sbom`
would fail CI otherwise.
...
An SBOM is the formal inventory of everything a piece of software is made of: every
dependency, direct and transitive, with its version, licence and origin. It exists to
answer questions like "does this release contain library X at the vulnerable version?"
quickly, without rebuilding the dependency tree by hand. GitHub has a good introduction in
[What is an SBOM?](https://github.com/resources/articles/what-is-an-sbom-software-bill-of-materials).

This project publishes its SBOM in **[CycloneDX](https://cyclonedx.org/) 1.6 (JSON)**,
the OWASP / ECMA-424 standard most analysis tools read:

- **[`sbom.cdx.json`](sbom.cdx.json)** in the repository root, describing the current
  `main` branch;
- **`garmin-mcp-ts.sbom.cdx.json`** attached to every release, describing exactly the
  bundle published beside it:

  ```bash
  wget https://github.com/sedoglia/garmin-mcp-ts/releases/latest/download/garmin-mcp-ts.sbom.cdx.json
  ```

The SBOM lists only the dependencies that end up in the `.mcpb` bundle — the production
ones, plus the optional `keytar` — resolved from `package-lock.json`. For each component
it records name, version, declared licence,
[Package URL](https://github.com/package-url/purl-spec) (`pkg:npm/axios@1.20.0`) and the
SHA-512 hash of the npm tarball; the `dependencies` section keeps the tree, so each
transitive dependency can be traced back to the direct one that pulls it in. Development
dependencies (TypeScript, tsx, the packer, the generator itself) are absent for the same
reason they are absent from the bundle: no user ever loads them.

The file is generated by [`@cyclonedx/cyclonedx-npm`](https://github.com/CycloneDX/cyclonedx-node-npm)
in reproducible mode: no serial number and no timestamp, so it only changes when a
dependency does. Regenerate it with:

```bash
npm run sbom
```

`npm run check:sbom` fails CI when `sbom.cdx.json` no longer matches the lockfile, just
as `check:manifest` does for the manifest; the release workflow checks it too before
attaching it.

Any tool that reads CycloneDX can query it. Two examples with vulnerability scanners that
work straight from the SBOM, no `node_modules` required:
