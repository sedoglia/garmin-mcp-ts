...
## [4.6.0] - 2026-09-21 — Bundle SBOM

No change to the tools, which remain 110. Every release from here on publishes the
inventory of the dependencies it ships, and the bundle is rebuilt with dotenv 18.

### ✨ SBOM in CycloneDX format
- **[`sbom.cdx.json`](sbom.cdx.json)** lists what goes into the `.mcpb` bundle: the 139
  production and optional dependencies resolved from `package-lock.json`, each with its
  declared licence, Package URL and tarball hash (CycloneDX 1.6). It is generated by
  `@cyclonedx/cyclonedx-npm` in reproducible mode, so it only changes when a dependency
  does.
- The release workflow attaches it beside `.mcpb` and `.sha256` as
  **`garmin-mcp-ts.sbom.cdx.json`**: it describes exactly the bundle of that release and
  can be fed to `grype` or `trivy` without installing anything.
- `npm run check:sbom` fails CI when the file no longer matches the lockfile, the same
  way `check:manifest` already does for the manifest.
- `package.json` gains the `license`, `repository`, `homepage` and `bugs` fields: without
  them the SBOM's root component carried no licence or references.
- Both READMEs get a dedicated section with the commands to verify, regenerate and scan
  the file.

### 📦 Version updates
- **dotenv** 17.4.2 → 18.0.0 (major): `.env.vault` support and `node -r dotenv/config`
  preloading are gone; the `dotenv/config` import stays, and the "injected env" message
  moves from stdout to stderr — one risk fewer for a server on stdio. The server keeps
  calling `config({ processEnv, quiet })` as before and its behaviour is unchanged.
- **@types/node** 26.4.1 → 26.6.1 (dev only).
